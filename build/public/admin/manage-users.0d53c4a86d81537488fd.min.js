"use strict";(self.webpackChunknodebb=self.webpackChunknodebb||[]).push([[30160,50970,70565,22852,83014,28253,99617,80560,73382,10999,86193],{54582:(R,P,u)=>{var p,h;p=[u(32230),u(59006),u(86193),u(74344),u(22568),u(63281),u(7927),u(23884),u(59265),u(55299)],h=function(E,x,C,g,D,d,s,y,v,b){const w={};w.init=function(){$("#results-per-page").val(ajaxify.data.resultsPerPage).on("change",function(){const n=utils.params();n.resultsPerPage=$("#results-per-page").val();const t=M(n);ajaxify.go(window.location.pathname+"?"+t)}),$(".export-csv").on("click",function(){return socket.once("event:export-users-csv",function(){s.remove("export-users-start"),s.alert({alert_id:"export-users",type:"success",title:"[[global:alert.success]]",message:"[[admin/manage/users:export-users-completed]]",clickfn:function(){window.location.href=config.relative_path+"/api/admin/users/csv"},timeout:0})}),socket.emit("admin.user.exportUsersCSV",{},function(n){if(n)return s.error(n);s.alert({alert_id:"export-users-start",message:"[[admin/manage/users:export-users-started]]",timeout:ajaxify.data.userCount/5e3*500})}),!1});function i(){const n=[];return $('.users-table [component="user/select/single"]').each(function(){$(this).is(":checked")&&n.push($(this).attr("data-uid"))}),n}function l(n,t){$('.users-table [component="user/select/single"]:checked').parents(".user-row").find(n).each(function(){$(this).toggleClass("hidden",!t)})}function I(){$('.users-table [component="user/select/single"]').prop("checked",!1),$('.users-table [component="user/select/all"]').prop("checked",!1)}function S(n){const t=document.querySelector(`.users-table [component="user/select/single"][data-uid="${n}"]`);if(t){const r=t.closest(".user-row");r.parentNode.removeChild(r)}}function T(n,t,r){return function(o){if(o)return s.error(o);s.success(n),t&&l(t,r),I()}}function U(n,t,r){s.success(n),t&&l(t,r),I()}$('[component="user/select/all"]').on("click",function(){$('.users-table [component="user/select/single"]').prop("checked",$(this).is(":checked"))}),$(".manage-groups").on("click",function(){const n=i();if(!n.length)return s.error("[[error:no-users-selected]]"),!1;socket.emit("admin.user.loadGroups",n,function(t,r){if(t)return s.error(t);x.render("admin/partials/manage_user_groups",r).then(function(o){const m=d.dialog({message:o,title:"[[admin/manage/users:manage-groups]]",onEscape:!0});m.on("shown.bs.modal",function(){C.group(m.find(".group-search"),function(k,A){const N=$(k.target).attr("data-uid");g.put("/groups/"+A.item.group.slug+"/membership/"+N,void 0).then(()=>{A.item.group.nameEscaped=E.escape(A.item.group.displayName),app.parseAndTranslate("admin/partials/manage_user_groups",{users:[{groups:[A.item.group]}]},function(K){$("[data-uid="+N+"] .group-area").append(K.find(".group-area").html())})}).catch(s.error)})}),m.on("click",".group-area a",function(){m.modal("hide")}),m.on("click",".remove-group-icon",function(){const k=$(this).parents("[data-group-name]"),A=k.attr("data-group-name"),N=$(this).parents("[data-uid]").attr("data-uid");return g.del("/groups/"+D(A)+"/membership/"+N).then(()=>{k.remove()}).catch(s.error),!1})})})}),$(".set-reputation").on("click",function(){const n=i();if(!n.length)return s.error("[[error:no-users-selected]]"),!1;let t="";if(n.length===1){const o=ajaxify.data.users.find(m=>m&&m.uid===parseInt(n[0],10));o&&(t=String(o.reputation))}const r=d.dialog({message:`<input id="new-reputation" type="text" class="form-control" value="${t}">`,title:"[[admin/manage/users:set-reputation]]",onEscape:!0,buttons:{submit:{label:"[[global:save]]",callback:function(){const o=r.find("#new-reputation").val();if(!utils.isNumber(o))return s.error("[[error:invalid-data]]"),!1;socket.emit("admin.user.setReputation",{value:o,uids:n}).then(()=>{n.forEach(m=>{$(`[component="user/reputation"][data-uid="${m}"]`).text(v.formattedNumber(o));const k=ajaxify.data.users.find(A=>A&&A.uid===parseInt(m,10));k&&(k.reputation=o)})}).catch(s.error)}}}});r.on("shown.bs.modal",()=>{r.find("#new-reputation").selectRange(0,r.find("#new-reputation").val().length)})}),$(".ban-user").on("click",function(){const n=i();if(!n.length)return s.error("[[error:no-users-selected]]"),!1;d.confirm(n.length>1?"[[admin/manage/users:alerts.confirm-ban-multi]]":"[[admin/manage/users:alerts.confirm-ban]]",function(t){t&&Promise.all(n.map(function(r){return g.put("/users/"+r+"/ban")})).then(()=>{U("[[admin/manage/users:alerts.ban-success]]",".ban",!0)}).catch(s.error)})}),$(".ban-user-temporary").on("click",function(){const n=i();if(!n.length)return s.error("[[error:no-users-selected]]"),!1;x.render("modals/temporary-ban",{}).then(function(t){d.dialog({className:"ban-modal",title:"[[user:ban-account]]",message:t,show:!0,buttons:{close:{label:"[[global:close]]",className:"btn-link"},submit:{label:"[[admin/manage/users:alerts.button-ban-x, "+n.length+"]]",callback:function(){const r=$(".ban-modal form").serializeArray().reduce(function(m,k){return m[k.name]=k.value,m},{}),o=r.length>0?Date.now()+r.length*1e3*60*60*(parseInt(r.unit,10)?24:1):0;Promise.all(n.map(function(m){return g.put("/users/"+m+"/ban",{until:o,reason:r.reason})})).then(()=>{U("[[admin/manage/users:alerts.ban-success]]",".ban",!0)}).catch(s.error)}}}})})}),$(".unban-user").on("click",function(){const n=i();if(!n.length)return s.error("[[error:no-users-selected]]"),!1;Promise.all(n.map(function(t){return g.del("/users/"+t+"/ban")})).then(()=>{U("[[admin/manage/users:alerts.unban-success]]",".ban",!1)})}),$(".reset-lockout").on("click",function(){const n=i();n.length&&socket.emit("admin.user.resetLockouts",n,T("[[admin/manage/users:alerts.lockout-reset-success]]"))}),$(".change-email").on("click",function(){const n=i();if(n.length!==1)return s.error("[[admin/manage/users:alerts.select-a-single-user-to-change-email]]");b.init({uid:n[0],onSuccess:function(t){l(".notvalidated",!1),l(".pending",!1),l(".expired",!1),l(".validated",!1),l(".validated-by-admin",!!t),l(".no-email",!t),$('.users-table [component="user/select/single"]:checked').parents(".user-row").find(".validated-by-admin .email").text(t)}})}),$(".validate-email").on("click",function(){const n=i();n.length&&d.confirm("[[admin/manage/users:alerts.confirm-validate-email]]",function(t){t&&socket.emit("admin.user.validateEmail",n,function(r){if(r)return s.error(r);s.success("[[admin/manage/users:alerts.validate-email-success]]"),l(".notvalidated",!1),l(".pending",!1),l(".expired",!1),l(".validated",!1),l(".validated-by-admin",!0),I()})})}),$(".send-validation-email").on("click",function(){const n=i();n.length&&socket.emit("admin.user.sendValidationEmail",n,function(t){if(t)return s.error(t);s.success("[[notifications:email-confirm-sent]]")})}),$(".change-password").on("click",async function(){const n=i();if(!n.length)return;async function t(o){const m=o.find("#newPassword").val(),k=o.find("#confirmPassword").val();if(m!==k)throw new Error("[[[user:change-password-error-match]]");await Promise.all(n.map(A=>g.put("/users/"+A+"/password",{currentPassword:"",newPassword:m})))}const r=d.dialog({message:`<div class="d-flex flex-column gap-2">
				...
				</div>`,title:"[[admin/manage/users:change-password]]",onEscape:!0,buttons:{cancel:{label:"[[admin/manage/users:alerts.button-cancel]]",className:"btn-link"},change:{label:"[[admin/manage/users:alerts.button-change]]",className:"btn-primary",callback:function(){return t(r).then(()=>{r.modal("hide")}).catch(s.error),!1}}}})}),$(".password-reset-email").on("click",function(){const n=i();n.length&&d.confirm("[[admin/manage/users:alerts.password-reset-confirm]]",function(t){t&&socket.emit("admin.user.sendPasswordResetEmail",n,T("[[admin/manage/users:alerts.password-reset-email-sent]]"))})}),$(".force-password-reset").on("click",function(){const n=i();n.length&&d.confirm("[[admin/manage/users:alerts.confirm-force-password-reset]]",function(t){t&&socket.emit("admin.user.forcePasswordReset",n,T("[[admin/manage/users:alerts.validate-force-password-reset-success]]"))})}),$(".delete-user").on("click",()=>{F("[[admin/manage/users:alerts.confirm-delete]]","/account")}),$(".delete-user-content").on("click",()=>{F("[[admin/manage/users:alerts.confirm-delete-content]]","/content")}),$(".delete-user-and-content").on("click",()=>{F("[[admin/manage/users:alerts.confirm-purge]]","")});const H=document.querySelector(".users-table"),j=document.getElementById("action-dropdown");H.addEventListener("change",n=>{(n.target.closest('[component="user/select/single"]')||n.target.closest('[component="user/select/all"]'))&&(i().length?j.removeAttribute("disabled"):j.setAttribute("disabled","disabled"))});function F(n,t){const r=i();r.length&&d.confirm(n,function(o){o&&Promise.all(r.map(m=>g.del(`/users/${m}${t}`,{}).then(()=>{t!=="/content"&&S(m)}))).then(()=>{t!=="/content"?s.success("[[admin/manage/users:alerts.delete-success]]"):s.success("[[admin/manage/users:alerts.delete-content-success]]"),I(),$('.users-table [component="user/select/single"]').length||ajaxify.refresh()}).catch(s.error)})}function J(){$('[data-action="create"]').on("click",function(){return x.render("admin/partials/create_user_modal",{}).then(function(n){const t=d.dialog({message:n,title:"[[admin/manage/users:alerts.create]]",onEscape:!0,buttons:{cancel:{label:"[[admin/manage/users:alerts.button-cancel]]",className:"btn-link"},create:{label:"[[admin/manage/users:alerts.button-create]]",className:"btn-primary",callback:function(){return e.call(this),!1}}}});t.on("shown.bs.modal",function(){t.find("#create-user-name").focus()})}),!1})}function e(){const n=this,t=document.getElementById("create-user-name").value,r=document.getElementById("create-user-email").value,o=document.getElementById("create-user-password").value,m=document.getElementById("create-user-password-again").value,k=$("#create-modal-error");if(o!==m)return k.translateHtml("[[admin/manage/users:alerts.error-x, [[admin/manage/users:alerts.error-passwords-different]]]]").removeClass("hide");const A={username:t,email:r,password:o};g.post("/users",A).then(()=>{n.modal("hide"),n.on("hidden.bs.modal",function(){ajaxify.refresh()}),s.success("[[admin/manage/users:alerts.create-success]]")}).catch(N=>k.translateHtml("[[admin/manage/users:alerts.error-x, "+N.message+"]]").removeClass("hidden"))}a(),J(),L(),B(),y.handle()};function a(){function i(){$(".fa-spinner").removeClass("hidden"),f({searchBy:$("#user-search-by").val(),query:$("#user-search").val(),page:1})}$("#user-search").on("keyup",utils.debounce(i,250)),$("#user-search-by").on("change",i)}function f(i){const l=utils.params();l.searchBy=i.searchBy,l.query=i.query,l.page=i.page,l.sortBy=l.sortBy||"lastonline";const I=decodeURIComponent($.param(l));$.get(config.relative_path+"/api/admin/manage/users?"+I,function(S){c(S);const T=config.relative_path+"/admin/manage/users?"+I;history.pushState&&history.pushState({url:T},null,window.location.protocol+"//"+window.location.host+T)}).fail(function(S){S&&S.responseJSON&&S.responseJSON.error&&s.error(S.responseJSON.error)})}function c(i){x.render("partials/paginator",{pagination:i.pagination}).then(function(l){$(".pagination-container").replaceWith(l)}),app.parseAndTranslate("admin/manage/users","users",i,function(l){if($(".users-table tbody tr").remove(),$(".users-table tbody").append(l),l.find(".timeago").timeago(),$(".fa-spinner").addClass("hidden"),!$("#user-search").val()){$("#user-found-notify").addClass("hidden"),$("#user-notfound-notify").addClass("hidden");return}i&&i.users.length===0?($("#user-notfound-notify").translateHtml("[[admin/manage/users:search.not-found]]").removeClass("hidden"),$("#user-found-notify").addClass("hidden")):($("#user-found-notify").translateHtml(E.compile("admin/manage/users:alerts.x-users-found",i.matchCount,i.timing)).removeClass("hidden"),$("#user-notfound-notify").addClass("hidden"))})}function M(i){return $("#user-search").val()?(i.query=$("#user-search").val(),i.searchBy=$("#user-search-by").val()):(delete i.query,delete i.searchBy),decodeURIComponent($.param(i))}function L(){$(".users-table thead th").on("click",function(){const l=$(this).attr("data-sort");if(!l)return;const I=utils.params();I.sortBy=l,ajaxify.data.sortBy===l?I.sortDirection=ajaxify.data.reverse?"asc":"desc":I.sortDirection="desc";const S=M(I);ajaxify.go("admin/manage/users?"+S)})}function _(){const i=[];return $("#filter-by").find("[data-filter-by]").each(function(){$(this).find(".fa-check").length&&i.push($(this).attr("data-filter-by"))}),i}function B(){let i=_();$("#filter-by").on("click","li",function(){const l=$(this);return l.find("i").toggleClass("fa-check",!l.find("i").hasClass("fa-check")),!1}),$("#filter-by").on("hidden.bs.dropdown",function(){const l=_();let I=l.length!==i.length;if(l.length===i.length&&l.forEach(function(S,T){S!==i[T]&&(I=!0)}),i=_(),I){const S=utils.params();S.filters=l;const T=M(S);ajaxify.go("admin/manage/users?"+T)}})}return w}.apply(P,p),h!==void 0&&(R.exports=h)},55299:(R,P,u)=>{var p,h;p=[u(74344),u(63281),u(7927)],h=function(E,x,C){const g={};return g.init=function(D){const d=x.dialog({message:`
